...
-- | Confusion matrix over a labelled dataset. Entry @(t, p)@ is the number of
-- examples whose true label is @t@ and whose prediction is @p@. Rows index the
-- true label, columns the predicted one; the diagonal holds the correct
-- predictions.
--
-- Predictions are computed only once (the @preds@ list is shared across all
-- cells through laziness), so the cost is @n@ forward passes plus an
-- @O(o² · n)@ count over already-materialised pairs.
confusionMatrix
  :: (KnownNat i, KnownNat h, KnownNat o)
  => Network i h o
  -> [(Vec i Double, Int)]     -- ^ labelled examples (label = class index)
  -> Mat o o Int
