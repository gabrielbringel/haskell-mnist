...
implementation with a property-based test suite: $17$ QuickCheck properties, each
...
importantly---the backward passes. Finite-difference checking of a backward pass
has a direct precedent in typed Haskell: Grenade's test suite compares a
backpropagated derivative with a finite-difference
perturbation~\cite{campbell2016grenade}. Our three gradient properties enumerate
the selected tensors per-element. \texttt{prop\_crossentropy\_grad\_fd} verifies
the gradient of cross-entropy with respect to the network's raw logits (before
softmax): for each logit entry, it perturbs by \(\pm\varepsilon\) and compares the
resulting central finite difference of \(L = \mathrm{crossEntropy}(\mathrm{softmax}(\mathbf{z})\ \mathbf{y})\)
against the analytical gradient component. To prevent softmax underflow---which
would make \(\log 0 = -\infty\) and break the finite-difference computation---raw
logits are compressed to the band \([-4, 4]\) via \(\mathbf{z} = 4 \cdot \tanh(\mathbf{z}_{\mathrm{raw}} / 4)\), ensuring all softmax probabilities remain well-conditioned (\(\gtrsim 3 \times 10^{-5}\)). \texttt{prop\_layer\_backward\_dW} perturbs each
...
The network property discards cases whose perturbation crosses a hidden
pre-activation of zero: ReLU is not differentiable there, so a central
difference across the kink is not comparable to the implementation's stated
convention \(\mathrm{ReLU}'(0)=0\).
...
The finite-difference tests use a step size \(\varepsilon = 10^{-6}\) and a
scale-aware tolerance \(|a - n| < 10^{-4}(1 + |a|) + 10^{-3}\) to account for
the wide range of weight magnitudes produced by QuickCheck's arbitrary
generators. Test networks are reduced-dimension instances of the main
architecture (4 inputs, 3 hidden, 2 outputs) for tractable property enumeration.
All code, results, and experiment logs are archived in a public GitHub repository:
\url{https://github.com/gabrielbringel/haskell-mnist}. The MNIST dataset must be
obtained separately: download the four IDX-format files
(\texttt{train-images-idx3-ubyte}, \texttt{train-labels-idx1-ubyte},
...
images, 2049 for labels) and confirms each image is 28~$\times$~28 pixels.
