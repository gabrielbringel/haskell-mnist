...
in a property-based test suite of $17$ QuickCheck properties, and, as an
end-to-end functional check, train the network on MNIST to $95.49\%$ test
accuracy. The contribution is expository: it makes legible and
...
differences. As an end-to-end functional check beyond those targeted tests, the
network trains on MNIST to $95.49\%$ test accuracy with no automatic
differentiation and no BLAS (\Cref{sec:results}).
...
\begin{itemize}
  ...
  \item a per-element verification of the hand-derived gradients
    ...
    ($95.49\%$ on MNIST) demonstrating that the typed implementation trains.
\end{itemize}
...
their guarantees are scoped to the framework operations and language features
modeled by the analyzer, and they remain separate from the type checker the
programmer already relies on.
...
through unchecked. A formal account of gradual tensor shape checking likewise
combines dimensions known statically with dimensions left
dynamic~\cite{eremondi2023graten}.
...
BLAS~\cite{hmatrix}. Grenade's test suite provides a direct precedent for
checking a typed backward pass numerically: it compares a backpropagated
derivative with a finite-difference perturbation~\cite{campbell2016grenade}.
Our small fixed-size setting lets us make that check exhaustive over the
selected tensors: each individual weight entry is perturbed and checked, so a
failure localizes to a specific component. A second difference of degree is
that our implementation depends on no linear-algebra library at all, so every
...
underlying vector. Crucially, the \texttt{Vec} and \texttt{Mat} constructors are
not exported; values can be constructed only through \emph{smart constructors}---\texttt{generate}, \texttt{replicate}, \texttt{fromList} for \texttt{Vec}, and \texttt{mgenerate}, \texttt{mreplicate}, \texttt{mfromList} for \texttt{Mat}---each of which establishes and verifies the size invariant. This abstraction guarantees that a well-typed value of \texttt{Vec}~\(n\) or \texttt{Mat}~\(r\)~\(c\) cannot store the wrong number of elements: there is no form to construct them incorrectly.

Two of these constructors, \texttt{fromList} and \texttt{mfromList}, are the boundary-facing pair: they build a shaped value from a plain list whose length is not statically known, so they return \texttt{Maybe} rather than assuming success. This is precisely where the type-level guarantee meets untrusted external data---for instance, the MNIST loader turns each image's raw byte payload into a \texttt{Vec 784} through \texttt{fromList}, so a malformed file yields \texttt{Nothing} at the boundary instead of a silently mis-sized vector.

The operation that makes these types earn their keep is the
...
Without a tensor framework's runtime checks, a wrong dimension can surface far
from its cause, as a failure in a later multiplication. Encoding the dimension
in the type moves the check to the definition site.
...
backward pass reuses the forward pass's guarantees: an incompatible transpose
or contraction cannot satisfy the surrounding gradient types. As elsewhere,
types rule out shape errors, not numerically wrong expressions that happen to
...
implementation with a property-based test suite of $17$ QuickCheck properties,
each run on $100$ randomized test cases by default. All properties pass both in
the recorded default run and in a $1\,000$-case stress run.

\paragraph{The 17 properties: design and enumeration.}
The test suite is organized into five categories: shaped linear algebra, activations
and loss, linear layers, full network composition, and evaluation metrics.
\Cref{tab:properties} lists all seventeen properties with the forms they check.

\begin{table*}[t]
  \centering
  \caption{The 17 QuickCheck properties. Each runs on $100$ randomized inputs
    (or $100$ cases with automatic discard). Dimensions vary by property, chosen
    for tractable enumeration rather than shared across the whole suite: the
    network- and confusion-matrix-level properties use a reduced $4 \to 3 \to 2$
    instance of the architecture (cf.\ \Cref{sec:res:limits}), the single-layer
    properties use an independent reduced $3 \to 4$ layer, the loss properties
    run at the true $10$-class output size, and the remaining linear-algebra
    properties use other small, arbitrary-sized instances (see
    \Cref{sec:res:correct} for exact shapes). All entries are verified
    component-by-component where applicable.
    Finite-difference tests use central differences with step
    $\varepsilon = 10^{-6}$; see text for tolerance formulas.}
  \label{tab:properties}
  \begin{tabular}{l l}
    \toprule
    \textbf{Property Name} & \textbf{Validates} \\
    \midrule
    \multicolumn{2}{l}{\emph{Linear Algebra}} \\
    \texttt{prop\_transpose\_involution} & $\mathrm{transpose}(\mathrm{transpose}(M)) = M$ \\
    \texttt{prop\_dot\_comm} & $\mathbf{u} \cdot \mathbf{v} = \mathbf{v} \cdot \mathbf{u}$ (exact) \\
    \texttt{prop\_outer\_elements} & $\mathtt{outer}(\mathbf{u}, \mathbf{v})[i,j] = u_i v_j$ \\
    \texttt{prop\_mulV\_naive} & Matrix--vector product vs.\ list reference \\
    \midrule
    \multicolumn{2}{l}{\emph{Activations \& Loss}} \\
    \texttt{prop\_softmax\_sums\_to\_one} & $\sum_i \mathrm{softmax}(\mathbf{z})_i = 1$ \\
    \texttt{prop\_relu\_nonneg} & $\mathrm{ReLU}(x) \ge 0$ \\
    \texttt{prop\_relu\_deriv} & $\mathrm{ReLU}'(x) = \begin{cases} 1 & x > 0 \\ 0 & x \le 0 \end{cases}$ \\
    \texttt{prop\_crossentropy\_nonneg} & $L = -\sum_i y_i \log p_i \ge 0$ \\
    \texttt{prop\_softmax\_crossentropy\_grad\_fd} & Gradient of $\mathrm{crossEntropy}(\mathrm{softmax}(\mathbf{z}), \mathbf{y})$ \\
    \midrule
    \multicolumn{2}{l}{\emph{Linear Layer}} \\
    \texttt{prop\_layer\_forward\_linear} & Forward pass linearity: $f(\alpha \mathbf{x}) = \alpha f(\mathbf{x})$ \\
    \texttt{prop\_layer\_backward\_dW} & Layer weight gradient: $\mathrm{d}W = \mathrm{d}\mathbf{z} \otimes \mathbf{x}$ \\
    \texttt{prop\_layer\_backward\_dB} & Layer bias gradient: $\mathrm{d}\mathbf{b} = \mathrm{d}\mathbf{z}$ \\
    \midrule
    \multicolumn{2}{l}{\emph{Network \& Backpropagation}} \\
    \texttt{prop\_network\_forward\_zero} & Zero-weight network: $f(\mathbf{x}) = \mathbf{0}$ \\
    \texttt{prop\_network\_backward\_zero} & Zero-weight backprop: $\mathrm{d}W_1, \mathrm{d}\mathbf{b}_1, \mathrm{d}W_2 = \mathbf{0}$ \\
    \texttt{prop\_network\_backward\_fd} & Full network gradients vs.\ finite differences \\
    \midrule
    \multicolumn{2}{l}{\emph{Evaluation}} \\
    \texttt{prop\_confusion\_total} & Confusion matrix sum = dataset size \\
    \texttt{prop\_confusion\_diagonal} & Diagonal sum = number of correct predictions \\
    \bottomrule
  \end{tabular}
\end{table*}

\paragraph{Per-element finite-difference verification.}
Finite-difference checking of a backward pass has a direct precedent in typed
Haskell: Grenade's test suite compares backpropagated derivatives with
finite-difference perturbations~\cite{campbell2016grenade}. Our three gradient
properties enumerate the selected tensors per-element, a deliberate design choice
that localizes any discrepancy to the specific entry where it occurs rather than
confirming only an aggregate response. Each component is perturbed by $\pm\varepsilon$,
and the central finite difference $\bigl(L(\theta+\varepsilon) - L(\theta-\varepsilon)\bigr)/2\varepsilon$
is compared against the analytical gradient component.

\paragraph{Softmax and cross-entropy: numerical scheme.}
The loss is $L = \mathrm{crossEntropy}(\mathrm{softmax}(\mathbf{z}), \mathbf{y})$, where
$\mathbf{z}$ is a pre-softmax logit vector and $\mathbf{y}$ is one-hot. The softmax
is computed in its numerically stable form:
\begin{align}
  m &= \max_i z_i, \\
  p_i &= \frac{\exp(z_i - m)}{\sum_j \exp(z_j - m)},
\end{align}
and the cross-entropy loss is $L = -\sum_i y_i \log p_i$, with the convention that
$0 \log 0 = 0$ (omit terms where $y_i = 0$). No clipping is applied. A probability
$p_i$ can still underflow to zero, yielding $L = \infty$, which is numerically
correct but can destabilize finite-difference perturbations. To prevent this in
\texttt{prop\_softmax\_crossentropy\_grad\_fd}, raw logits are compressed to the band
$[-4, 4]$ via the preventive bounding $\mathbf{z} = 4 \cdot \tanh(\mathbf{z}_{\mathrm{raw}} / 4)$,
ensuring all softmax probabilities remain well-conditioned ($\gtrsim 3 \times 10^{-5}$).
The network backpropagation property (\texttt{prop\_network\_backward\_fd}) does
\emph{not} apply this compression. Instead, every component of its weights,
biases, and inputs is drawn independently and uniformly from $[-1,1]$. For the
reduced $4\to3\to2$ network this keeps the resulting logits in a well-conditioned
range while exercising the actual, uncompressed softmax implementation.

\paragraph{Gradient properties: layer and network.}
\texttt{prop\_layer\_backward\_dW} verifies each individual weight entry $W[i,j]$ of
a single reduced $3 \to 4$ layer (weight matrix \texttt{Mat 4 3}) against the
surrogate loss $L = \mathrm{d}\mathbf{z} \cdot \mathbf{z}$.
\texttt{prop\_network\_backward\_fd} extends this to the full two-layer network and
verifies every element of the four parameter tensors ($W_1, \mathbf{b}_1, W_2, \mathbf{b}_2$):
\begin{itemize}
  \item $W_1$: $3 \times 4 = 12$ elements,
  \item $\mathbf{b}_1$: $3$ elements,
  \item $W_2$: $2 \times 3 = 6$ elements,
  \item $\mathbf{b}_2$: $2$ elements,
\end{itemize}
for a total of $23$ gradient components per test case. The full network property
propagates each perturbation through the complete forward and backward pass against
the loss $L = \mathrm{crossEntropy}(\mathrm{softmax}(\mathbf{z}_2), \mathbf{y})$,
verifying that \Cref{sec:backprop}'s hand-derived gradients match the definition of
the derivative rather than being assumed.

\paragraph{Handling non-differentiable points.}
The network property discards test cases whose perturbation crosses a hidden
pre-activation of zero: ReLU is not differentiable at zero, so a central difference
across the kink is not comparable to the implementation's stated convention
$\mathrm{ReLU}'(0) = 0$. The discard filter is:
\begin{align}
  \text{Keep case if:} \quad
  |z_{1,i}| > 2\varepsilon\max(1,\lVert\mathbf{x}\rVert_\infty)
  \quad \forall i.
\end{align}
This margin covers both a hidden-bias perturbation of size $\varepsilon$ and the
largest hidden-weight perturbation $\varepsilon |x_j|$, with a factor of two for
rounding. No cases were discarded in either the recorded $100$-case run or the
$1\,000$-case stress run.

\paragraph{Numerical parameters.}
For the full-network gradient property, weights, biases, and inputs have
independent components drawn uniformly from $[-1,1]$, and class labels are
uniformly sampled from $\{0,1\}$. The other properties use QuickCheck's standard
\texttt{Arbitrary} generators, with the explicit logit compression described
above where needed. The finite-difference step size is $\varepsilon = 10^{-6}$.
Two scale-aware comparison tolerances are used:
\begin{itemize}
  \item \texttt{prop\_softmax\_crossentropy\_grad\_fd} and
    \texttt{prop\_layer\_backward\_dW}:
    $|a - n| < 10^{-4}(1 + |a|) + 10^{-3}$;
  \item \texttt{prop\_network\_backward\_fd}:
    $|a - n| \le 10^{-6} + 10^{-4} \cdot \max(|a|, |n|)$.
\end{itemize}
These tolerances account for floating-point rounding at the different numerical
scales exercised by the two generator regimes. The default suite uses $100$
successful cases per property; setting \texttt{QC\_CASES=1000} runs the recorded
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\columnwidth]{fig-training.pdf}
...
at epoch $6$), as online, batch-size-one SGD with a fixed learning rate need not
settle monotonically. For a from-scratch network with no
mini-batching and no BLAS, reaching ${\sim}95\%$ is a functional sanity check
that the implementation behaves coherently end to end, not a proof of numerical
correctness or a competitive claim.
...
$7\!\to\!9$ ($21$), and $5\!\to\!8$ ($20$). The most revealing pattern is the
column for the digit $8$: it absorbs $127$ false positives---$2$, $3$, $5$, and
$9$ are all commonly misread as $8$ in this run---giving it the lowest
precision of any class ($\sim\!87.9\%$). By recall the hardest digits are $7$
($93.8\%$), $6$ ($94.0\%$), and $2$ ($94.3\%$), while $0$ ($98.9\%$) and $1$
($98.7\%$) are the easiest.
...
\paragraph{Reproducibility and numerical scheme.}
Raw MNIST pixel bytes (0--255) are normalized to the interval \([0, 1]\) by
dividing by 255.0. The training set is processed in its original order for every
epoch with no reshuffling between epochs; this is a limitation of the training
procedure that may lead to oscillations in accuracy, as observed in the reported
runs (e.g., dips from epoch~3's plateau to lower values in epochs~6 and beyond).
The finite-difference tests use a step size \(\varepsilon = 10^{-6}\) and scale-aware
tolerances detailed in \Cref{sec:res:correct}. Test networks are reduced-dimension
instances of the main architecture (4 inputs, 3 hidden, 2 outputs) for tractable
property enumeration. All code, results, and experiment logs are archived in a public
GitHub repository: \url{https://github.com/gabrielbringel/haskell-mnist}. The MNIST
dataset~\cite{lecun-mnist} must be obtained separately: download the four IDX-format
files below from the MNIST archive, decompress them with \texttt{gunzip} if
necessary, and place them in a \texttt{data/} directory:
\begin{itemize}
  \item \texttt{train-images-idx3-ubyte}
  \item \texttt{train-labels-idx1-ubyte}
  \item \texttt{t10k-images-idx3-ubyte}
  \item \texttt{t10k-labels-idx1-ubyte}
\end{itemize}
The loader validates the IDX magic numbers (2051 for
images, 2049 for labels) and confirms each image is 28~$\times$~28 pixels. Exact
SHA-256 checksums of the four uncompressed files used for the reported run are
recorded in \texttt{results/run-manifest.json} and checked by
\texttt{scripts/validate\_results.py}.
...
inputs (\Cref{sec:res:correct}). As a broader functional check, the network
trains on MNIST to ${\sim}95\%$ test accuracy with no
automatic-differentiation library and no BLAS (\Cref{fig:training}). The broader
takeaway is that a mainstream functional language, without a specialized
dependent type theory, is already expressive enough to make this class of
tensor-dimension mismatch unrepresentable, and to do so without hiding the
mathematics inside a framework.
...
emphasis on type-level correctness comes at the cost of throughput: boxed
vectors, online updates, and no BLAS are not a design for competitive training
speed. These are boundaries of the demonstration, not of the technique. The
