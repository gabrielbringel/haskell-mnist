...
end-to-end confirmation, train the typed network on MNIST to ${\sim}90\%$ test
...
trains on MNIST to ${\sim}90\%$ test accuracy with no automatic differentiation
...
\begin{itemize}
  ...
  \item a property-based verification of the hand-derived gradients against
    ...
    (${\sim}90\%$ on MNIST) confirming the typed implementation trains correctly.
\end{itemize}
...
(\Cref{sec:res:limits}). \Cref{tab:setup} lists the configuration shared by all
runs.

\begin{table}[t]
  \centering
  \caption{Experimental configuration. All runs share these settings; only the
    learning rate is varied (\Cref{sec:res:training}).}
  \label{tab:setup}
  \begin{tabular}{ll}
    \toprule
    Setting & Value \\
    ...
    Examples / epoch   & $10\,000$ (subset of $60\,000$) \\
    Test examples      & $2\,000$ (subset of $10\,000$) \\
    Epochs             & $5$ \\
    RNG seed           & $42$ \\
    Runtime            & ${\sim}7.5$ min / run (GHC 9.4.8, \texttt{-O2}) \\
    \bottomrule
  \end{tabular}
\end{table}
...
implementation with a property-based test suite: $14$ QuickCheck properties, each
...
\begin{figure*}[t]
  ...
  \caption{Mean training loss (a) and test accuracy (b) per epoch for the two
    learning rates. The larger rate ($0.10$) oscillates and stalls; the smaller
    rate ($0.05$) descends smoothly and reaches ${\sim}90\%$. Epoch $0$ in (b) is
    the accuracy at initialization ($9.90\%$), shared by both runs.}
  \label{fig:training}
\end{figure*}

To confirm that the typed network trains as an ordinary classifier, we run it on
MNIST with the default learning rate of $0.05$. Test accuracy rises from
$9.90\%$ at initialization---near the $1/10$ chance rate, confirming a
near-uniform softmax output before training---to a peak of $91.35\%$ at epoch $4$,
finishing at $89.75\%$ after five epochs, while the mean per-example
cross-entropy falls monotonically from $0.53$ to $0.23$ (\Cref{fig:training}).
For a from-scratch network with no mini-batching and no BLAS, reaching
${\sim}90\%$ is a functional sanity check that the typed forward and backward
passes are correct end to end, not a competitive claim.

The learning rate is the one hyperparameter that decides whether the network
converges, which we show by comparing $0.05$ against $0.10$ under an otherwise
identical protocol. At $0.10$ the loss stalls around $0.94$ and accuracy
oscillates between $70\%$ and $78\%$ across epochs: for batch-size-one SGD, the
larger step overshoots and the parameters bounce around a minimum instead of
settling into it. Halving the rate to $0.05$ removes the oscillation and yields
the smooth descent above. This is the paper's single ablation, and it isolates
the step size as the decisive factor.
...
\begin{figure}[t]
  ...
  \caption{Confusion matrix on the test subset for the final $\text{lr}=0.05$
    network (epoch $5$, ${\sim}89.75\%$). Color encodes the row-normalized
    frequency; annotations are raw counts.}
  \label{fig:confusion}
\end{figure}

\Cref{fig:confusion} shows the confusion matrix of the final $\text{lr}=0.05$
network on the test subset. Most of the mass lies on the diagonal, consistent
with the ${\sim}90\%$ accuracy. The off-diagonal errors are not spread uniformly;
\Cref{fig:confusion} lets us read off which digit pairs dominate the residual
error for this run.
% TODO: after generating results/confusion-matrix.csv, name the top 2-3 confused
% pairs and add one sentence interpreting them (MNIST commonly confuses visually
% similar pairs such as 4/9, 3/5, 7/9). Replace this comment with the real ones.

\subsection{Scope and limitations}
\label{sec:res:limits}

The evaluation is deliberately modest, matching the project's emphasis on
type-level correctness over throughput. We train on a $10\,000$-image subset and
evaluate on a $2\,000$-image subset rather than the full sets, to keep wall time
reasonable under a boxed-\texttt{Vector} implementation with no BLAS. We report a
single run per configuration at a fixed seed, so the curves in
\Cref{fig:training} carry no error bars; the learning-rate comparison should be
read as indicative rather than as a statistically controlled study. Finally,
training uses online SGD with one gradient step per image, not mini-batches, so
the loss curves are noisier than a batched optimizer would produce. None of these
choices bear on the correctness claims of \Cref{sec:res:correct}, which are
independent of the training subset or seed.
...
to ${\sim}90\%$ test accuracy with no automatic-differentiation library and no
...
two-layer classifier trained with online SGD on subsets of MNIST, and the
